...
# Note  : This version differs from the original in three ways (similar to tent.py): (1) it tracks running stats for BN
#         to enable frozen inference, (2) it drops batch-wise episodic resetting and multi-step gradient updates, and 
#         (3) it allows prediction after adaptation via reforward. It's identical to the original during adaptation (for
#         predict-then-adapt). The momentum parameter is only for BN layers.
class SAR(Method):
    ...
    def forward(self, x):
        ...
        with torch.enable_grad():
            ...
            # First forward: Filter reliable samples (low entropy)
            ...
            # Second forward: Filter reliable samples again at perturbed weights
